    ...
    <page-model>
      <template slot="controlQueryOps">
        <el-form :model="examine" label-width="86px" :inline="true" ref="controlQueryForm">
          ...
          <div class="search-create-time">
            <el-form-item label="录入时间" prop="startCreateTime">
    ...
    <el-dialog title="报备渠道选择" :visible.sync="performDisplay" width="600px">
      <el-form :model="performModelData" class="alert-model-form" label-width="100px">
        <el-form-item prop="name1" label="报备渠道" required>
          <el-select v-model="performModelData.name1" placeholder="请选择"  @change="reportSource">
            <el-option v-for="item in reportSourceData" :key="item.id" :label="item.channelName.text" :value="item.channelName.code"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  created() {
    this.getMchList();
    this.setStorage('merchants_operation', '{}');
    this.getReportData();
  },
  methods: {
    //查询
    ...
    performModelSubmit() {
      if(!this.channelNo){this.$message('请选择报备渠道!');return}
      window.console.log(this.performModelData);
      this.performDisplay = false;

      const data = this.getStorage('merchants_operation');
      data.channelNo = this.channelNo;
      this.setStorage('merchants_operation', data);
      this.$router.push('/merchants/list/operation/4');
    },
    // 新增1 + 预览0 + 商户更新2 + 商户审核3
    // 新增
    addClick() {
    ...
    // 报备渠道源数据
    getReportData() {
      fetch.mcChannelList().then(res => {
        const hash = {};
        const arr = res.data.list.reduce(function(item, next) {
            hash[next.channelName.text] ? '' : hash[next.channelName.text] = true && item.push(next);
            return item;
        }, []);
        this.reportSourceData = arr;
      });
    },
    reportSource(selVal){
      this.channelNo = selVal;
    },
     // 报备
    ReportedClick(row) {
      const data = { custId: row.custId };
