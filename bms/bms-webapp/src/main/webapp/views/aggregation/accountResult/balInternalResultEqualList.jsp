...
<body>
						...
						<div class="col-xs-12">
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									<div class="tab-pane fade in active" id="internal_result_equal">
							   			<form action='<c:url value="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALRESULTEQUALLIST%>"/>'method="post" id="searchForm">
											...
											<tr>
												...
												<td class="td-right" >
													...
													<select name="channelId" id="channelId">
														...
														<c:forEach items="${baseChannels}" var="channel">
															<option value="${channel.channelId }" <c:if test="${channel.channelId==balInternalResultEqual.channelId}">selected</c:if>>${channel.channelName }</option>
														</c:forEach>
													</select>
													</span>
												</td>
												...
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="clearId" id="clearId" value="${balInternalResultEqual.clearId }">
													</span>
												</td>
											</tr>
											<tr>
												...
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="batchId" id="batchId" value="${balInternalResultEqual.batchId }">
													</span>
												</td>
												<td class="td-left">会计日期</td>
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="beginWorkDate"  id="beginWorkDate" value="${balInternalResultEqual.beginWorkDate}" readonly="readonly" onfocus="AutoDatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
														-
														<input type="text" name="endWorkDate"  id="endWorkDate" value="${balInternalResultEqual.endWorkDate}" readonly="readonly" onfocus="AutoDatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
													</span>
												</td>
											</tr>
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALRESULTEQUALLIST %>">
														...
														<gyzbadmin:function url="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALRESULTEQUALEXPORT%>">
										...
										<div class="list-table-header">聚合对账结果一致表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<tbody>
													<c:forEach items="${resultList }" var="balInternalResultEqual">
														<tr class="statistic">
															<td>${balInternalResultEqual.equalId }</td>
															<td>${balInternalResultEqual.dataId }</td>
															<td>${balInternalResultEqual.channelName }</td>
															<td>${balInternalResultEqual.clearId }</td>
															<td>${balInternalResultEqual.workDate }</td>
															<td>${balInternalResultEqual.batchId }</td>
															<td>${balInternalResultEqual.balResultName }</td>
															<td>${balInternalResultEqual.balMemo }</td>
															<td>${balInternalResultEqual.instDate }</td>
														</tr>
													</c:forEach>
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ AccountResultPath.BASE+AccountResultPath.BALINTERNALRESULTEQUALEXPORT%>?beginWorkDate="+
			beginWorkDate+
				"&channelId="+
				channelId+
				"&clearId="+
