...
<body>
						...
						<div class="col-xs-12">
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									<div class="tab-pane fade in active" id="external_result_exception">
							   			<form action='<c:url value="<%=AccountResultPath.BASE + AccountResultPath.EXTERNALRESULTEXCEPTIONLIST%>"/>'method="post" id="searchForm">
											...
											<tr>
												...
												<td class="td-right" >
													...
													<select name="channelId" id="channelId">
														...
														<c:forEach items="${baseChannels}" var="channel">
															<option value="${channel.channelId }" <c:if test="${channel.channelId==balExternalResultException.channelId}">selected</c:if>>${channel.channelName }</option>
														</c:forEach>
													</select>
													</span>
												</td>
												...
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="clearId" id="clearId" value="${balExternalResultException.clearId }">
													</span>
												</td>
											</tr>
											<tr>
												...
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="batchId" id="batchId" value="${balExternalResultException.batchId }">
													</span>
												</td>
												<td class="td-left">会计日期</td>
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="beginWorkDate"  id="beginWorkDate" value="${balExternalResultException.beginWorkDate}" readonly="readonly" onfocus="AutoDatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
														-
														<input type="text" name="endWorkDate"  id="endWorkDate" value="${balExternalResultException.endWorkDate}" readonly="readonly" onfocus="AutoDatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
													</span>
												</td>
											</tr>
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=AccountResultPath.BASE + AccountResultPath.EXTERNALRESULTEXCEPTIONLIST %>">
										...
										<div class="list-table-header">外部系统对账结果一致表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<tbody>
													<c:forEach items="${resultList }" var="balExternalResultException">
														<tr class="statistic">
															<td>${balExternalResultException.exceptionId }</td>
															<td>${balExternalResultException.dataId }</td>
															<td>${balExternalResultException.channelName }</td>
															<td>${balExternalResultException.fileId }</td>
															<td>${balExternalResultException.seq }</td>
															<td>${balExternalResultException.clearId }</td>
															<td>${balExternalResultException.workDate }</td>
															<td>${balExternalResultException.batchId }</td>
															<td>${balExternalResultException.balResult }</td>
															<td>${balExternalResultException.balMemo }</td>
															<td>${balExternalResultException.instDate }</td>
															<td>${balExternalResultException.dealFlag }</td>
															<td>${balExternalResultException.dealMemo }</td>
															<td>${balExternalResultException.dealUser }</td>
															<td>${balExternalResultException.dealDatetime }</td>
														</tr>
													</c:forEach>
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ AccountResultPath.BASE+AccountResultPath.EXTERNALRESULTEXCEPTIONEXPORT%>?beginWorkDate="+
			beginWorkDate+
				"&channelId="+
				channelId+
				"&clearId="+
