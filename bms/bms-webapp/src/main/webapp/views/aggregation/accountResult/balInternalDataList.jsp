...
<body>
						...
						<div class="col-xs-12">
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									<div class="tab-pane fade in active" id="internal_data">
							   			<form action='<c:url value="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALDATALIST%>"/>'method="post" id="searchForm">
											<table class="search-table">
											<tr>
												<td class="td-left">源数据编号</td>
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="dataId" id="dataId" value="${balInternalData.dataId }">
													</span>
												</td>
												...
												<td class="td-right" >
													...
													<select name="channelId" id="channelId">
														...
														<c:forEach items="${baseChannels}" var="channel">
															<option value="${channel.channelId }" <c:if test="${channel.channelId==balInternalData.channelId}">selected</c:if>>${channel.channelName }</option>
														</c:forEach>
													</select>
													</span>
												</td>
											...
											<tr>
												...
												<td class="td-right">
													<span class="input-icon">
														<input type="text" name="clearId" id="clearId" value="${balInternalData.clearId }">
													</span>
												</td>
												<td class="td-left">会计日期</td>
												<td class="td-right" colspan="3">
													<span class="input-icon">
														<input type="text" name="workDate"  id="workDate" value="${balInternalData.workDate}" readonly="readonly" onfocus="AutoDatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
													</span>
												</td>
											</tr>
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALDATALIST %>">
														...
														<gyzbadmin:function url="<%=AccountResultPath.BASE + AccountResultPath.BALINTERNALDATAEXPORT%>">
										...
										<div class="list-table-header">聚合源数据表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<c:forEach items="${resultList }" var="externalData">
														<tr class="statistic">
															...
															<td width="500">${externalData.details}</td>
														</tr>
													</c:forEach>
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ AccountResultPath.BASE+AccountResultPath.BALINTERNALDATAEXPORT%>?dataId="+
				dataId+
				"&channelId="+
				channelId+
				"&clearId="+
