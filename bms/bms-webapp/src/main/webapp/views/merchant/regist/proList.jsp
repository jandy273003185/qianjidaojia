...
if(!isPhoneNo($("#updateMerchant  #representativeMobile")[0])){
	...
	var businessPhotoImageVal02=$("#businessPhotoImageVal02").val();
	var businessPhoto=$("#businessPhoto").val();
	if(businessPhoto==""){
		businessPhotoImageVal02="";
	}
	var doorPhotoImageVal02=$("#doorPhotoImageVal02").val();
	var doorPhoto=$("#doorPhoto").val();
	if(doorPhoto==""){
		doorPhotoImageVal02="";
	}
    var certAttribute0Val02=$("#certAttribute0Val02").val();
    var certAttribute0=$("#certAttribute0").val();
    if(certAttribute0==""){
    	certAttribute0Val02="";
    }
	var certAttribute1Val02=$("#certAttribute1Val02").val();
	 var certAttribute1=$("#certAttribute1").val();
        if(certAttribute1==""){
        	certAttribute1Val02="";
        }
	var certAttribute2Val02=$("#certAttribute2Val02").val();
	var certAttribute2=$("#certAttribute2").val();
        if(certAttribute2==""){
        	certAttribute2Val02="";
        }
				...
				<div class="page-content">
					<div class="row">
						...
						<div id="door_temp">
							...
							<!-- 查询条件 -->
						<form  id="merchantForm" action='<c:url value="<%=MerchantPath.BASE + MerchantPath.PROLIST %>"/>' method="post">
								...
								<tr>
									...
									<td colspan="6" align="center">
										<span class="input-group-btn">
											<gyzbadmin:function url="<%=MerchantPath.BASE + MerchantPath.PROLIST %>">
										...
										<tr class="merchant" id="merchant">
											...
											<td>	
												...
												<%-- <gyzbadmin:function url="<%=MerchantPath.BASE + MerchantPath.SENDEMAIL %>"> 
													<c:choose>   
								  						<c:when test="${merchant.state =='01' or merchant.state =='04'}">  									  
                                            	 ...
                                            	 <c:choose>
                                            	 	<c:when test="${firstAudit=='YES' and (merchant.workFlowStatus=='04' or merchant.workFlowStatus=='03')}">
                                            	 		<button type="button" onclick="updateMerchantInfo(this,'firstAudit')" id="btnFirstAudit" class="btn btn-primary btn-xs"  >一级审核</button>	
                                            	 	</c:when>
                                            	 	<c:otherwise>
                                            	 		<button type="button"  id="btnFirstAudit" class="btn  btn-xs"  disabled="disabled" >一级审核</button>	
                                            	 	</c:otherwise>
                                            	 </c:choose>
                                            	 
                                            	  <c:choose>
                                            	 	<c:when test="${secondAudit=='YES' and merchant.workFlowStatus=='01'}">
                                            	 		<button type="button" onclick="updateMerchantInfo(this,'secondAudit')" id="btnSecondAudit" class="btn btn-primary btn-xs"  >二级审核</button>	
                                            	 	</c:when>
                                            	 	<c:otherwise>
                                            	 		<button type="button"  id="btnSecondAudit" class="btn  btn-xs"  disabled="disabled" >二级审核</button>	
                                            	 	</c:otherwise>
                                            	 </c:choose>
		...
		<div class="modal fade" style="z-index:1043;" id="secondAuditMessageModel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		   <div class="modal-dialog" style="width:30%;z-index:99;">
		      <div class="modal-content" >
		         ...
		         <div class="modal-footer">
		            <button type="button" class="btn btn-default messageDefault" data-dismiss="modal">取消</button>
		            <button type="button" class="btn btn-primary addadBtn" onclick="secondNotPass()">确定</button>
		         </div>
		      </div><!-- /.modal-content -->
		     </div>
		</div><!-- /.modal -->
			...
			<div class="modal-dialog" style="width:60%;z-index:90;">
		      <div class="modal-content" style="width:950px;" id="merchantDiv">
		         ...
		         <div class="modal-body">
						...
						<tr>
						 ...
						 <tr>
							...
							<tr>
								...
								<td class="td-left numberHide">来往单位编号：</td>
								<td class="td-right numberHide_">
						...
						<tr>
						   ...
						   <%-- 	<tr>
							...
							<tr class="commentHide">
								<td colspan="4" class="headlerPreview">修改备注</td></tr>
							<tr>
							<tr class="commentHideTarea">
		         ...
		         <div class="modal-footer">
		            	...
		            	<button type="button" class="btn btn-primary firstAuditNotPassBtn" data-toggle='modal'  data-target="#firstAuditMessageModel" >一级审核不通过</button>
		            	<button type="button" class="btn btn-primary firstAuditPassBtn" onclick="firstPass()">一级审核通过</button>
		            	<button type="button" class="btn btn-primary secondAuditNotPassBtn" data-toggle='modal'  data-target="#secondAuditMessageModel">二级审核不通过</button>
		            	<button type="button" class="btn btn-primary secondAuditPassBtn" onclick="secondPass()">二级审核通过</button>
		         </div>
		      </div><!-- /.modal-content -->
		   </div>
		</div>
...
function secondPass(){
    var fcontactunitNumber = $("#updateMerchant #fcontactunitNumber").val();
    var isClear=$("#isClear").val().trim();
	if(kong.test(fcontactunitNumber)) {
		$.gyzbadmin.alertFailure("请填写来往单位编号！");
		return false;
	} 
	if(kong.test(isClear)) {
		$.gyzbadmin.alertFailure("请选择是否直清！");
		return false;
	} 
	...
	$.ajax({
		type:"POST",
		dataType:"json",
		url:window.Constants.ContextPath +'<%=MerchantPath.BASE+ MerchantPath.SECONDPASS %>',
		data:
		{
			"number" :fcontactunitNumber,
			"custId" : custId,
			"isClear" :isClear
		},
		success:function(data){
...
function secondNotPass(){
	var custId = $("#updateMerchant #custName_id_01").val();
	
	$.ajax({
		type:"POST",
		dataType:"json",
		url:window.Constants.ContextPath +'<%=MerchantPath.BASE+ MerchantPath.SECONDNOTPASS %>',
		data:
		{
			"custId" 	: custId,
			"message"	: $("#secondAuditMessageModel #auditMessage").val()
		},
		success:function(data){
			if(data.result=="SUCCESS"){
				$.gyzbadmin.alertSuccess("审核完成！",function(){
...
function firstNotPass(){
	...
	$.ajax({
		type:"POST",
		dataType:"json",
		url:window.Constants.ContextPath +'<%=MerchantPath.BASE+ MerchantPath.FIRSTNOTPASS %>',
		data:
		{
			"custId" 	: custId,
			"message"	: $("#firstAuditMessageModel #auditMessage").val()
		},
		success:function(data){
			if(data.result=="SUCCESS"){
				$.gyzbadmin.alertSuccess("审核完成！",function(){
...
function firstPass(){
	...
	/* var feeCode  = $('#updateMerchant input[name="rule"]:checked').val(); */
	var custId = $("#updateMerchant #custName_id_01").val();
	
	$.ajax({
		type:"POST",
		dataType:"json",
		url:window.Constants.ContextPath +'<%=MerchantPath.BASE+ MerchantPath.FIRSTPASS %>',
		data:
		{
			"custId" 	: custId,
		 	"isClear" : isClear 
...
function updateMerchantInfo(obj,option){
	...
	if(option=='edit'){
		$('#updateMerchant').on('show.bs.modal', function () {
			...
			$('#updateMerchant .numberHide').show();
			$('#updateMerchant .numberHide_').show();
			$('#updateMerchant input,select').prop("disabled", false);
		});
		
	/* 	$('#updateMerchant').on('hide.bs.modal', function () {
			$('#updateMerchant .updateMerchantBtn').show();
			$('#updateMerchant .updateImageDiv').show();
			$('#updateMerchant .firstAuditNotPassBtn').show();
			$('#updateMerchant .firstAuditPassBtn').show();
			$('#updateMerchant .secondAuditNotPassBtn').show();
			$('#updateMerchant .secondAuditPassBtn').show();
		}); */
	...
	if(option=="firstAudit"){
		
		$('#updateMerchant').on('show.bs.modal', function () {
			$('#updateMerchant .closeBtn').hide();
			$('#updateMerchant .updateMerchantBtn').hide();
			$('#updateMerchant .updateImageDiv').hide();
			$('#updateMerchant .firstAuditNotPassBtn').show();
			$('#updateMerchant .firstAuditPassBtn').show();
			...
			$("select[id='isClear']").attr("disabled", false);
			
			
		});
		/* $('#updateMerchant').on('hide.bs.modal', function () {
			$('#updateMerchant .closeBtn').show();
			$('#updateMerchant .updateMerchantBtn').show();
			$('#updateMerchant .updateImageDiv').show();
			$('#updateMerchant .firstAuditNotPassBtn').show();
			$('#updateMerchant .firstAuditPassBtn').show();
			...
			$('#updateMerchant .commentHide').show();
			$('#updateMerchant .commentHideTarea').show();
			$('#updateMerchant .numberHide').show();
			$('#updateMerchant .numberHide_').show();
		}); */
	...
	$("#updateMerchant #doorPhotoImageDiv").attr("src","<%=request.getContextPath()+AuditorPath.BASE+ AuditorPath.IMAGE %>?custId="+custId+"&certifyType=08&authId="+authId);
	...
	$.ajax({
		...
		/* $("#updateMerchant #openAddressRec").val(data.merchantVo.openAddressRec); */
		/* // 初始化城市
		...
		$("input[name='rule'][value='"+data.merchantVo.feeCode+"']").attr("checked",true);
			if($("input[name='rule'][value='"+data.merchantVo.feeCode+"']").prop("class")=='Package'){
				$("#updateMerchant #ruleB").find("option[value='Package']").attr("selected",true);  
				$(".PackageTr").show();
				$(".FixedTr").hide();	
			}else{
				$("#updateMerchant #ruleB").find("option[value='Fixed']").attr("selected",true);  
				$(".PackageTr").hide();
				$(".FixedTr").show();
			} */
		}
...
/* $("#updateMerchant #ruleB").change(function(){
	$(".PackageTr").hide();
	$(".FixedTr").hide();
