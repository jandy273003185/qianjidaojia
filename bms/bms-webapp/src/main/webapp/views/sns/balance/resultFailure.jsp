...
<body>
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									...
									<div class="tab-pane fade in active">
							   			<form action='<c:url value="<%=RedenvResultPath.BASE + RedenvResultPath.RESULT_FAILURE%>"/>'method="post" id="searchForm">
											...
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.RESULT_FAILURE%>">
														...
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.RESULT_FAILURE_EXPORT%>">
										...
										<div class="list-table-header">差错报表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<tbody>
													<c:forEach items="${resultFailureList}" var="resultFailure">
														<tr class="resultFailure">
															<td>${resultFailure.batchId}</td>
															<td>${resultFailure.instDate}</td>
															<td>${resultFailure.clearId}</td>
															<td>${resultFailure.workDate}</td>
															<td>${resultFailure.custId}</td>
															<td>${resultFailure.transType}</td>
															<td>${resultFailure.transAmt}</td>
															<td>${resultFailure.transStatus}</td>
															<td>${resultFailure.reserve}</td>
															<td>${resultFailure.balMemo}</td>
														</tr>
													</c:forEach>
													<c:if test="${empty resultFailureList}">
														<tr><td colspan="10" align="center"><font style="color: red; font-weight: bold;font-size: 15px;">暂无数据</font></td></tr>
													</c:if>
													</tbody>
												</table>
												<!-- 分页 -->
												<c:if test="${not empty resultFailureList}">
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ RedenvResultPath.BASE+RedenvResultPath.RESULT_FAILURE_EXPORT%>?clearId="+
				clearId+
				"&instDate="+
				instDate+
				"&workBeginDate="+
