...
<body>
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									...
									<div class="tab-pane fade in active">
							   			<form action='<c:url value="<%=RedenvResultPath.BASE + RedenvResultPath.SEVEN_DOUBT%>"/>'method="post" id="searchForm">
											...
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.SEVEN_DOUBT%>">
														...
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.SEVEN_DOUBT_EXPORT%>">
										...
										<div class="list-table-header">七分钱存疑报表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<tbody>
													<c:forEach items="${sevenDoubtList}" var="sevenDoubt">
														<tr class="sevenDoubt">
															<td>${sevenDoubt.batchId}</td>
															<td>${sevenDoubt.instDate}</td>
															<td>${sevenDoubt.clearId}</td>
															<td>${sevenDoubt.workDate}</td>
															<td>${sevenDoubt.custId}</td>
															<td>${sevenDoubt.transCode}</td>
															<td>${sevenDoubt.transAmt}</td>
															<td>${sevenDoubt.transStatus}</td>
															<td>${sevenDoubt.balMemo}</td>
														</tr>
													</c:forEach>
													<c:if test="${empty sevenDoubtList}">
														<tr><td colspan="9" align="center"><font style="color: red; font-weight: bold;font-size: 15px;">暂无数据</font></td></tr>
													</c:if>
													</tbody>
												</table>
												<!-- 分页 -->
												<c:if test="${not empty sevenDoubtList}">
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ RedenvResultPath.BASE+RedenvResultPath.SEVEN_DOUBT_EXPORT%>?clearId="+
				clearId+
				"&instDate="+
				instDate+
				"&workBeginDate="+
