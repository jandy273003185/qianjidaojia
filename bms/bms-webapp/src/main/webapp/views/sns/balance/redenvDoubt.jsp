...
<body>
							...
							<div class="table-responsive">
								<div id="myTabContent" class="tab-content">
									...
									<div class="tab-pane fade in active">
							   			<form action='<c:url value="<%=RedenvResultPath.BASE + RedenvResultPath.REDENV_DOUBT%>"/>'method="post" id="searchForm">
											...
											<tr>
												<td colspan="6" align="center">
													<span class="input-group-btn">
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.REDENV_DOUBT%>">
														...
														<gyzbadmin:function url="<%=RedenvResultPath.BASE + RedenvResultPath.REDENV_DOUBT_EXPORT%>">
										...
										<div class="list-table-header">红包存疑报表</div>
											<div class="table-responsive">
												<table id="sample-table-2" class="list-table">
													...
													<tbody>
													<c:forEach items="${redenvDoubtList}" var="redenvDoubt">
														<tr class="redenvDoubt">
															<td>${redenvDoubt.batchId}</td>
															<td>${redenvDoubt.instDate}</td>
															<td>${redenvDoubt.clearId}</td>
															<td>${redenvDoubt.workDate}</td>
															<td>${redenvDoubt.custId}</td>
															<td>${redenvDoubt.transType}</td>
															<td>${redenvDoubt.transAmt}</td>
															<td>${redenvDoubt.transStatus}</td>
															<td>${redenvDoubt.reserve}</td>
															<td>${redenvDoubt.balMemo}</td>
														</tr>
													</c:forEach>
													<c:if test="${empty redenvDoubtList}">
														<tr><td colspan="10" align="center"><font style="color: red; font-weight: bold;font-size: 15px;">暂无数据</font></td></tr>
													</c:if>
													</tbody>
												</table>
												<!-- 分页 -->
												<c:if test="${not empty redenvDoubtList}">
	...
	$(function () {	
		...
		$(".exportBut").click(function(){
			...
			var src ="<%= request.getContextPath()+ RedenvResultPath.BASE+RedenvResultPath.REDENV_DOUBT_EXPORT%>?clearId="+
				clearId+
				"&instDate="+
				instDate+
				"&workBeginDate="+
