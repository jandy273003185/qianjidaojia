...
<%@page import="com.qifenqian.bms.myworkspace.overAudit.OverAuditPath"%>
...
$(function(){
	...
	$('.btn-sm').click(function(){
		var startDate =$("#auditBeginTime").val();
		var endDate= $("#auditEndTime").val();
		if("" != startDate && "" != endDate && startDate > endDate) 
		{
			alert("结束日期不能小于开始日期");
			return false;
		}
		var form = $('#overAuditForm');
		form.submit();
	});
		...
		<div class="main-container-inner">
			...
			<div class="main-content">
				...
				<div class="page-content">
					...
					<div class="row">
						<form action='<c:url value="<%=OverAuditPath.BASE+OverAuditPath.LIST%>"/>' method="post" id="overAuditForm">
							<table class="search-table">
								<tr>
									...
									<td class="td-left">审核日期</td>
									<td class="td-right">
										 <input type="text" name="auditBeginTime"  id="auditBeginTime" readonly="readonly" value="${auditBeginTime }" onfocus="WdatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
											-
										 <input type="text" name="auditEndTime" id="auditEndTime" readonly="readonly" value="${auditEndTime }" onfocus="WdatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;">
									</td>
								</tr>
								<tr>
						...
						<div class="col-xs-12">
							...
							<div class="table-responsive">
								<table id="sample-table-2" class="list-table">
									...
									<tbody>
										<c:forEach items="${overAudits}" var="overAudit">
											<tr >
												<td>
												
												<a href="<%=request.getContextPath()%>${overAudit.url }?processInstanceId=${overAudit.procInstId}">${overAudit.id }</a>
												</td>
												<td>${overAudit.procInstId }</td>
												<td>
													${overAudit.name }
												</td>
												<td>${overAudit.proName }</td>
												<td>${overAudit.realName}</td>
												<td>
												<fmt:formatDate value="${overAudit.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
												</td>
												<td>
												<fmt:formatDate value="${overAudit.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
												</td>
												
											</tr>
										</c:forEach>
										
										<c:if test="${empty overAudits}">
							...
							<!-- 分页 -->
							<c:if test="${not empty overAudits}">
