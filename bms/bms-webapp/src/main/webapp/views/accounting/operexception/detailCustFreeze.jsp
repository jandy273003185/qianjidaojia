...
<%@page import="com.qifenqian.bms.accounting.exception.dao.operfreeze.bean.OperFreeze"%>
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<!-- 业务基本信息 -->
							<%
							OperFreeze operation = (OperFreeze)pageContext.findAttribute("operation");
							%>
							<table class="search-table">
								<tr>
									<td class="bg-e" colspan="6" align="center" style="border-left:0px;">
										...
										<c:if test="${operation.freezeStatus != '00' and operation.freezeStatus != '09'}">
								...
								<tr>
									...
									<td class="td-right" width="23%">
										<c:choose>
											<c:when test="${operation.freezeStatus == '00'}">成功</c:when>
											<c:when test="${operation.freezeStatus == '01'}">待提交</c:when>
											<c:when test="${operation.freezeStatus == '02'}">提交核心处理</c:when>
											<c:when test="${operation.freezeStatus == '03'}">核心处理失败</c:when>
											<c:when test="${operation.freezeStatus == '99'}">取消</c:when>
											<c:otherwise>${operation.freezeStatus}</c:otherwise>
										</c:choose>
									</td>
								</tr>
								<tr>
									...
									<td class="td-left bg-e">客户账号</td>
									<td class="td-right">${operation.acctId}</td>
								...
								<tr>
									...
									<td class="td-right">${operation.relateId}</td>
									<td class="td-left bg-e">核心返回码</td>
									<td class="td-right">${operation.rtnCode}</td>
									<td class="td-left bg-e">摘要</td>
									<td class="td-right">${operation.brief}</td>
								</tr>
							...
							<table class="search-table" style="margin-top: 10px;">
								...
								<c:forEach items="${transList }" var="trans" varStatus="status">
									...
									<!-- 交易明细 -->
									<%
									  if(trans.getTransFlowOperate() == RequestColumnValues.TransFlowOperate.JGKJ_CARD_FREEZE||
									...
									<%
									  } if(trans.getTransFlowOperate()==RequestColumnValues.TransFlowOperate.SEVEN_CUST_FULL_FREEZE||
									...
									<tr>
										...
										<td class="td-right">${trans.relateId}</td>
										<td class="td-left bg-e">收支标记</td>
										<td class="td-right"><%=ReflectUtils.getDesc(trans.getTransRecordFlow().getLoanFlag()) %></td>
									</tr>
									<tr>
