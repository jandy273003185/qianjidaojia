...
<body onload="loadBankCardPaymentRefund()">
	<!-- 用户信息 -->
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<!-- 业务基本信息 -->
							<%
								OperRefund operation = (OperRefund)pageContext.findAttribute("operation");
							%>
							<table class="search-table">
								<tr>
									<td class="bg-e" colspan="6" align="center" style="border-left:0px;">
										...
										<c:if test="${operation.refundState !='04'||operation.refundState != '00'}">
								...
								<tr>
									...
									<td class="td-right" width="23%">
										<c:choose>
											<c:when test="${operation.refundState == '00'}">退款成功</c:when>
											<c:when test="${operation.refundState == '02'}">核心处理中</c:when>
											<c:when test="${operation.refundState == '03'}">核心处理中</c:when>
											<c:when test="${operation.refundState == '04'}">退款失败(取消)</c:when>
											<c:otherwise>${operation.refundState}</c:otherwise>
										</c:choose>
									</td>
								</tr>
								<tr>
									<td class="td-left bg-e">退款申请时间</td>
								...
								<tr>
									...
									<td class="td-left bg-e">原商户订单号</td>
									<td class="td-right">${operation.originMerOrderId}</td>
									<td class="td-left bg-e">原始交易时间</td>
									<td class="td-right"><fmt:formatDate value="${operation.originTransTime}" type="both"/></td>
								</tr>
								<tr>
									<td class="td-left bg-e">原始交易金额</td>
									<td class="td-right">${operation.originTransAmt}</td>
									<td class="td-left bg-e">退款理由</td>
									<td class="td-right" colspan="3">${operation.refundMemo}</td>
								</tr>
								<tr>
									<td class="td-left bg-e">退款方客户编号</td>
									<td class="td-right">${operation.refundCustId}</td>
									<td class="td-left bg-e">商户客户号</td>
									<td class="td-right">${operation.merchantCustId}</td>
									<td class="td-left bg-e">原交易报文编号</td>
									<td class="td-right" >${operation.originCoreSn}</td>
								</tr>
								<tr>
									...
									<td class="td-right" >
										<c:if test="${operation.originCoreSn=='00'}">事前已统计RN</c:if>
										<c:if test="${operation.originCoreSn=='01'}">事前已统计RN</c:if>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="td-right"><fmt:formatDate value="${operation.coreReturnTime}" type="both"/></td>
									...
									<td class="td-right"><fmt:formatDate value="${operation.verificationTime}" type="both"/></td>
								</tr>
							</table>
							<!-- 核心基本信息 -->
							...
							<table class="search-table" style="margin-top: 10px;">
								...
								<c:forEach items="${transList }" var="trans" varStatus="status">
									...
									<!-- 交易明细 -->
									<%
									  if(Arrays.asList(RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE,
																													RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE_REVOKE,
																													RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE_REFUND,
																													RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT,
																													RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT_REVOKE,
																													RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT_REFUND).contains(trans.getTransFlowOperate())) {
									...
									%>
										...
										<td class="bg-e" colspan="4" width="67%" align="right" style="padding-right: 50px;border-left:0px;">
											...
											%>
												...
												%>
														<%
														  if(trans.getTransFlowOperate()==RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE_REFUND){
														%>
												...
												%>
													...
													<%
													  if(trans.getTransFlowOperate()==RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT_REFUND||
									...
									%>
										...
										<td class="bg-e" colspan="4" width="67%" align="right" style="padding-right: 50px;border-left:0px;">
											...
											%>
												...
												%>
													<%
													  if(trans.getTransFlowOperate() == RequestColumnValues.TransFlowOperate.JGKJ_PAYMENT_REFUND){
													%>
														<gyzbadmin:function url="<%=OperationExceptionPath.BASE + OperationExceptionPath.SEQUEL_NEXT_STEP_OPERATION%>">
															<a href="#transBaseModal"  class="tooltip-success sequelNextStepOperation" data-rel="tooltip"  data-toggle='modal' title="续作下一步">
																<button type="submit" class="btn btn-purple btn-sm">续作下一步</button>
										...
										<%
										  }else if(trans.getTransFlowOperate() == RequestColumnValues.TransFlowOperate.UNIONPAY_PAYMENT||
										...
										<tr>
											...
											<td class="td-right" width="23%" style="color: red">
												<c:choose>
										...
										<tr>
											...
											<td class="td-left bg-e">绑定标识号</td>
											<td class="td-right">${trans.bindId}</td>
											<td class="td-left bg-e">撤销查询流水号</td>
											<td class="td-right">${trans.revokeQueryId}</td>
										</tr>
										<tr>
											<td class="td-left bg-e">交易币别</td>
											<td class="td-right">${trans.transCurrCode}</td>
											<td class="td-left bg-e">交易金额</td>
										...
										<tr>
											...
											<td class="td-right" colspan="3">${trans.memo}</td>
										...
										<tr>
											...
											<td class="td-left bg-e">支付卡标识</td>
											<td class="td-right">${trans.payCardNo}</td>
											<td class="td-left bg-e">支付卡名称</td>
											<td class="td-right">${trans.payCardIssueName}</td>
										</tr>
										<tr>
											<td class="td-left bg-e">支付方式</td>
											<td class="td-right">${trans.payType}</td>
											<td class="td-left bg-e">汇率</td>
											<td class="td-right">${trans.exchangeRate}</td>
											<td class="td-left bg-e">兑换日期</td>
											<td class="td-right">${trans.exchangeDate}</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td class="td-left bg-e">VPC交易信息域</td>
											<td class="td-right">${trans.vpcTransData}</td>
											<td class="td-left bg-e">保留域</td>
											<td class="td-right">${trans.reserved}</td>
										</tr>
									<% } %>
								</c:forEach>
								</table>
								...
								<tr>
									...
									<td class="td-right" style="width:35%">
										<select id="chargeNetpayState" name="chargeNetpayState" clasS="width-90">
										  <option value="04">-退款失败(取消)-</option>
	...
	<script type="text/javascript">
	function loadBankCardPaymentRefund(){
