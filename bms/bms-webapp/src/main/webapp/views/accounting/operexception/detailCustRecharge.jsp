				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<!-- 业务基本信息 -->
							<%
								OperCharge operation = (OperCharge)pageContext.findAttribute("operation");
							%>
							...
							<tr>
								<td class="bg-e" colspan="6" align="center" style="border-left:0px;">
										...
										<c:if test="${operation.chargeNetpayState != '99' and operation.chargeNetpayState != '00'}">
								...
								<tr>
									...
									<td class="td-right" width="23%">
										<c:choose>
											<c:when test="${operation.chargeNetpayState == '01'}">待支付</c:when>
											<c:when test="${operation.chargeNetpayState == '02'}">银联执行成功</c:when>
											<c:when test="${operation.chargeNetpayState == '03'}">银联执行失败</c:when>
											<c:when test="${operation.chargeNetpayState == '04'}">核心处理中</c:when>
											<c:when test="${operation.chargeNetpayState == '05'}">核心处理失败</c:when>
											<c:when test="${operation.chargeNetpayState == '00'}">充值成功</c:when>
											<c:when test="${operation.chargeNetpayState == '99'}">已取消</c:when>
											<c:when test="${operation.chargeNetpayState == '90'}">未明</c:when>
											<c:otherwise>${operation.chargeNetpayState}</c:otherwise>
										</c:choose>
									</td>
								</tr>
								<tr>
									<td class="td-left bg-e">充值提交时间</td>
									<td class="td-right"><fmt:formatDate value="${operation.chargeSubmitTime}" type="both"/></td>
									<td class="td-left bg-e">充值金额</td>
									<td class="td-right" style="color: red">${operation.chargeAmt}</td>
								...
								<tr>
									<td class="td-left bg-e">充值返回时间</td>
									<td class="td-right"><fmt:formatDate value="${operation.chargeReturnTime}" type="both"/></td>
									<td class="td-left bg-e">客户号</td>
									<td class="td-right">${operation.custId}</td>
									<td class="td-left bg-e">客户名称</td>
									<td class="td-right">${operation.custName}</td>
								</tr>
								<tr>
									<td class="td-left bg-e">核心流水号</td>
									<td class="td-right">${operation.coreSn}</td>
									<td class="td-left bg-e">核心返回说明</td>
									<td class="td-right" >[${operation.coreReturnCode}]&nbsp;${operation.coreReturnMsg}</td>
									<td class="td-left bg-e">备注</td>
									<td class="td-right" >${operation.memo}</td>
								</tr>
							</table>
							<!-- 业务基本信息  end-->
