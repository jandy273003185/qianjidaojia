...
<body onload="loadBankCardPayment()">
	<!-- 用户信息 -->
						...
						<div class="col-xs-12">
							...
							<table class="search-table">
								...
								<tr>
									...
									<td class="td-left bg-e">商户订单号</td>
									<td class="td-right">${operation.merOrderId}</td>
									<td class="td-left bg-e">订单描述</td>
									<td class="td-right">${operation.orderDesc}</td>
								</tr>
								<tr>
									<td class="td-left bg-e">是否需要充值</td>
									<td class="td-right">
										<c:choose>
											<c:when test="${operation.isNeedCharge == '1'}">是</c:when>
											<c:when test="${operation.isNeedCharge == '0'}">否</c:when>
											<c:otherwise>${operation.isNeedCharge}</c:otherwise>
										</c:choose>
									</td>
									<td class="td-left bg-e">充值客户编号</td>
									<td class="td-right">${operation.chargeCustId}</td>
									<td class="td-left bg-e">充值流水号</td>
									<td class="td-right">${operation.chargeSn}</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="td-left bg-e">退款关联订单号</td>
									<td class="td-right">${operation.drawbackOrderId}</td>
								</tr>
								<tr>
									<td class="td-left bg-e">商家页面地址</td>
									<td class="td-right">${operation.pgUrl }</td>
									<td class="td-left bg-e">商家后台地址</td>
									<td class="td-right" colspan="3">${operation.bgUrl }</td>
								</tr>
								<tr>
							...
							<table class="search-table" style="margin-top: 10px;">
								...
								<c:forEach items="${transList }" var="trans" varStatus="status">
										...
										<tr>
											...
											<td class="bg-e" colspan="4" width="67%" align="right" style="padding-right: 50px;border-left:0px;">
												...
												%>
														...
														%>
															   <%
															     if (TransDealService.isOutDate(tranYl.getTransSubmitTime())){
															   %>
																	<gyzbadmin:function url="<%=OperationExceptionPath.BASE + OperationExceptionPath.ROLLBACK_OPERATION%>">
																		<a href="#transBaseModal"  data-toggle='modal' class="tooltip-success rollbackOperation" data-rel="tooltip" title="退回当前步骤" >
																			<button type="submit" class="btn btn-purple btn-sm" >退回当前步骤</button>
																		</a>
																	</gyzbadmin:function>
																<%
																  }else{
									...
									<%
									  } if(Arrays.asList(RequestColumnValues.TransFlowOperate.SEVEN_CUST_RECHARGE, 
																				    RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT,
																					RequestColumnValues.TransFlowOperate.SEVEN_CUST_RECHARGE_REVOKE, 
																					RequestColumnValues.TransFlowOperate.SEVEN_CUST_RECHARGE_REFUND, 
																					...
																					RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE,
																					RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE_REVOKE,
																					RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE_REFUND).contains(trans.getTransFlowOperate())) {
											...
											%>
												...
												%>
														<%
														  if(Arrays.asList(RequestColumnValues.TransFlowOperate.SEVEN_CUST_RECHARGE,
																														RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT,
																														RequestColumnValues.TransFlowOperate.SEVEN_BUSS_RECEIVE).contains(trans.getTransFlowOperate())){
														%>
															<gyzbadmin:function url="<%=OperationExceptionPath.BASE + OperationExceptionPath.REVOKE_TRANS%>">
																<a href="#transBaseModal"  class="tooltip-success revokeTrans" data-rel="tooltip"  data-toggle='modal' title="撤销当前步骤">
																	<button type="submit" class="btn btn-purple btn-sm">撤销当前步骤</button>
														...
														<%
														  if(Arrays.asList(RequestColumnValues.TransFlowOperate.SEVEN_CUST_RECHARGE,
																														RequestColumnValues.TransFlowOperate.SEVEN_CUST_PAYMENT).contains(trans.getTransFlowOperate())){
														%>
																<gyzbadmin:function url="<%=OperationExceptionPath.BASE + OperationExceptionPath.SEQUEL_NEXT_STEP_OPERATION%>">
																	<a href="#transBaseModal"  class="tooltip-success sequelNextStepOperation" data-rel="tooltip"  data-toggle='modal' title="续作下一步">
																		<button type="submit" class="btn btn-purple btn-sm">续作下一步</button>
