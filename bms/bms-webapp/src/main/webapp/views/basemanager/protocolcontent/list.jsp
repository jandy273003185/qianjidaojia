				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<!-- 查询条件 -->
							<form
								action='<c:url value="<%=ProtocolContentPath.BASE + ProtocolContentPath.LIST%>" />'
								...
								<input type="hidden" id="merchantCodeHd" value="${queryBean.merchantCode }" />
									<tr>
										...
										<td class="td-right">
											...
											<option value="">输入商户名或商户编号查询</option>
									...
									<tr>
										<td colspan="8" align="center">
											<span class="input-group-btn"> 
												<gyzbadmin:function url="<%=ProtocolContentPath.BASE + ProtocolContentPath.LIST%>">
										...
										<c:forEach items="${protocolContents}" var="protocolContent">
											<tr class="protocolContent">
												...
												<td>
													...
													<input name="custId" id="custId" type="hidden" value="${protocolContent.custId}"/>
													<input name="merCode" id="merCode" type="hidden" value="${protocolContent.merchantCode}"/>
					      ...
					      <div class="modal-content">
					         ...
					         <div class="modal-body">
					            <table class="modal-input-table" id="tbProtocolContent">
									<tr>
										<td class="td-left" width="30%">商户</td>
					   ...
					   <div class="modal-dialog">
					      <div class="modal-content" style="width: 600px;">
					         ...
					         <div class="modal-body" align="center">
					         	<font style="color: red; font-weight: bold; font-size: 15px;">您确定要删除该协议[<span class="id"></span>]么？</font>
					         </div>
					         <div class="modal-footer">
					         	<input type="hidden" name="id" class="id" value=""/>
					         	<input type="hidden" name="custId" id="custId" class="custId" value=""/>
					            <input type="hidden" name="merchantCode" id="merchantCode" class="merchantCode" value=""/>
...
function operateProtocol(v,type){
	...
	var custId = $(v).parent().find('input[name="custId"]').val();
	$("#previewProtocolContentModal #protocolContentId").val(protocolContentId);
	$("#previewProtocolContentModal #custId").val(custId);
	...
	for(var i=0; i<contentCount.length-1; i++){
		...
		if(contentCount[i].split(":").length>3){
			var index = contentCount[i].indexOf(':');
			index = contentCount[i].indexOf(":",index+1);
			$('#tbPreviewProtocolContent').append("<tr><td class='td-left'>"+contentCount[i].split(":")[0]+"</td><td class='td-right' colspan='3'><input type='text' class='width-100' id='content"+i+"' name='"+strName+"' value='"+contentCount[i].substring(index+1)+"'/></td></tr>");
		}
		else 
		if(strName == 'custManager'){
			...
			$('#tbPreviewProtocolContent').append(
				 ...
				 $("#previewProtocolContentModal #content"+i).val(contentCount[i].split(":")[2]);
		}/* else if(strName == 'compAcctBank'){
			$('#tbPreviewProtocolContent').append(
				 	"<tr><td class='td-left' width='30%'>"	+contentCount[i].split(":")[0]+"</td><td class='td-right' colspan='3'>"+
					"<sevenpay:selectAllBankTag id='content"+i+"' name ='"+strName+"'  clasS='width-60'/>"+
					"</td></tr>"
				);
				 $("#previewProtocolContentModal #content"+i).val(contentCount[i].split(":")[2]);
	...
	/* $("#tbPreviewProtocolContent").find('select[name="compAcctBank"]').comboSelect(); */
...
function firstPass(){
	var custId = $('#previewProtocolContentModal #custId').val();
	$.blockUI();
...
/* 	var protocolContents= '<c:out value="${gyzb:toJSONString(protocolContents)}" escapeXml="false"/>'; */
	var protocolContents= ${protocolContents};
...
/* 	$.each($.parseJSON(protocolContents),function(i,value){
	...
	$.each(protocolContents,function(i,value){
		$.data(protocolContentList[i],"protocolContent",value);
	});
        	...
        	if(data.result=="fail"){
				...
				for(var i=0; i<contentCount.length;i++){
				 if(contentCount[i].split(":").length>2){
						...
						}else if(contentCount[i].split(":")[0]=='payType'){
							$('#tbProtocolContent').append(
									"<tr><td class='td-left'>"+contentCount[i].split(":")[1]+"</td><td class='td-right' colspan='3'>"+
									"<select   id='content"+i+"' name='"+contentCount[i].split(":")[0]+"'>"+
											"<option value=''>--请选择--</option>"+
											"<option value='0' >付到银行卡</option>"+
											"<option value='1' selected ='selected'>付到其代付账户</option>"+
									"</select></td></tr>"
									);
						...
						}/* else if(contentCount[i].split(":")[0]=='compAcctBank'){
							$('#tbProtocolContent').append(
									"<tr><td class='td-left' width='30%'>"	+contentCount[i].split(":")[1]+"</td><td class='td-right' colspan='3'>"+
									"<sevenpay:selectAllBankTag id='content"+i+"' name ='"+contentCount[i].split(":")[0]+"'  clasS='width-60'/>"+
									"</td></tr>"
								);
						...
						/* $("#tbProtocolContent").find('select[name="compAcctBank"]').comboSelect(); */
	...
	$(".addProtocolContentBtn").click(function(){
			...
			for(var i=0; i<contentTempCount.length;i++){
				var	contents = trim($('#addProtocolContentModal #content'+i).val());
				 var arrRate = new Array('h5.gateway.pay_rate','mobile.plugin.pay_rate','onecode.coll.pay_rate','h5_t.gateway.pay_rate','pc.gateway.pay_rate','agent_merchant_rate');
				...
				if(contentTempCount[i].split(":")[0]=="custManager" || contentTempCount[i].split(":")[0]=="serviceProvider"){
				 ...
				 if('payType'==name){
					 	if(!isPayType(contents)){
					 		$.gyzbadmin.alertFailure(contentTempCount[i].split(":")[1]+'是否跳过付款方式不正确');
							return;
						}
					 }
	...
	$('.deleteProtocolContent').click(function(){
		...
		var custId = $(this).parent().find('input[name="custId"]').val();
		var merCode = $(this).parent().find('input[name="merCode"]').val();
		$('#deleteProtocolContentModal').on('show.bs.modal', function () {
			...
			$('#deleteProtocolContentModal input.custId').val(custId);
			$('#deleteProtocolContentModal input.merchantCode').val(merCode);
		});
		$('#deleteProtocolContentModal').on('hide.bs.modal', function () {
			...
			$('#deleteProtocolContentModal input.custId').val('');
			$('#deleteProtocolContentModal input.merchantCode').val('');
		});
	});
	...
	$('.deleteProtocolContentBtn').click(function(){
		...
		var custId = $('#deleteProtocolContentModal input.custId').val();
		var merCode = $('#deleteProtocolContentModal input.merchantCode').val();
		$.blockUI();
...
$("#previewProtocolContentModal #templateId").change(function(){
					...
					if(contentCount[i].split(":").length>2){
						...
						if(contentCount[i].split(":")[0]=='CNAPS'){
							...
							$('#tbProtocolContent').append(
									...
									"<select   id='content"+i+"' name='"+contentCount[i].split(":")[0]+"'>"+
											"<option value=''>--请选择--</option>"+
											"<option value='0' selected ='selected'>付到银行卡</option>"+
											"<option value='1'>付到其代付账户</option>"+
									"</select></td></tr>"
									);
						}else if(contentCount[i].split(":")[0]=='custManager'){
							$('#tbPreviewProtocolContent').append(
	...
	$("#previewProtocolContentModal .edit").click(function(){
		...
		for(var i=0; i<contentTempCount.length;i++){
			...
			var arrRate = new Array('h5.gateway.pay_rate','mobile.plugin.pay_rate','onecode.coll.pay_rate','h5_t.gateway.pay_rate','pc.gateway.pay_rate','agent.merchant.rate');
