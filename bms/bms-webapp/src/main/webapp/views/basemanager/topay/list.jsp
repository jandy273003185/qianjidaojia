			...
			<div class="main-content">
				...
				<div class="page-content">
					<div class="row">
						...
						<!-- 查询条件 -->
						<form  id="merchantForm" action='<c:url value="<%=TopayPath.BASE + TopayPath.LIST %>"/>' method="post">
							<input type="hidden" name="count" id="count" value="">					
							<input type="hidden" name="trade_Status" id="trade_Status" value="${queryBean.tradeStatus }">
							<input type="hidden" name="create_Id" id="create_Id" value="${queryBean.createId }">
								...
								-->	
									...
									<td class="td-right" >								   
										 <select name="tradeStatus" id="tradeStatus" >
											  ...
											  <option  value="08">已发送银行等待处理</option>
											  <option  value="99">提交银行失败 </option>
										 </select>
								...
								<tr>
									<td colspan="6" align="center">
										<span class="input-group-btn">
											<gyzbadmin:function url="<%=TopayPath.BASE + TopayPath.LIST%>">
									...
									<c:forEach items="${audingList}" var="payment" varStatus="i">
										...
										<!-- <td>${payment.createName }</td>		 -->									
											...
											<td>	
										 	    ...
										 	    <%-- <c:choose>
											 	     <c:when test="${payment.tradeStatus =='01'}">                                          	                                            	
		                                                 <button type="button" onclick="batchAuditPayment(this,'edit','Clste')"   class="btn btn-primary btn-xs qifenqian_view_tc updateMerchant1">财务审核</button>													                                       
	                                               	 </c:when>
	                                                 <c:otherwise>	                                            	
		                                                 <button type="button" onclick="batchAuditPayment(this,'edit','Clste')"   class="btn btn-primary btn-xs qifenqian_view_tc updateMerchant1">查看详情</button>													                                       
	                                                 </c:otherwise>
...
function batchAuditPayment(obj,type_1,audit_type) {	         
	         if(type_1 =='edit'){
		     var batNo=$(obj).parent().find('#batNo').val();		    
			 var url=window.Constants.ContextPath+"<%=TopayPath.BASE+ TopayPath.AUDITPAYMENTINFOS%>?batNo="+batNo; 			 
...
<script type="text/javascript">
 $(function(){
	...
	$(".firstAuditPassBtn").click(function(){
	 ...
	 var tradeStatus = $("#trade_Status").val();	
	 $("#createId option[value='"+createId+"']").attr("selected",true);
	 $("#tradeStatus option[value='"+tradeStatus+"']").attr("selected",true);
});
