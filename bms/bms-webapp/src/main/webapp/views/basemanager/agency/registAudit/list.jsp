...
<script type="text/javascript">
	$(function(){
		...
		$('.exportBut').click(function(){
			...
			var src ="<%= request.getContextPath()+ RegistAuditPath.BASE+RegistAuditPath.EXPORTMERCHANTINFO%>?merchantCode="+
			merchantCode+
			"&startCreateTime="+
			startCreateTime+
			"&endCreateTime="+
	...
	function openMerchentInfo(merchantCode){
		window.location.href="<%=RegistAuditPath.BASE + RegistAuditPath.PREVIEW%>?merchantCode="+merchantCode;
	}
				...
				<div class="page-content">
					<div class="row">
						...
						<input type="hidden" id="doorPhototemp" />
							<!-- 查询条件 -->
						<form  id="merchantForm" action='<c:url value="<%=RegistAuditPath.BASE + RegistAuditPath.LIST %>"/>' method="post">
							<table class="search-table">
								...
								<tr >
									...
									<td class="td-right">
									<input type="text" name="startCreateTime"   id="startCreateTime" readonly="readonly" value="${queryBean.startCreateTime }"  onfocus="WdatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(<%=request.getContextPath() %>/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;"> 
									-
									<input type="text" name="endCreateTime"   id="endCreateTime" readonly="readonly" value="${queryBean.endCreateTime }"  onfocus="WdatePicker({skin:'whyGreen',maxDate:'%y-%M-%d'})" style="background:#fff url(<%=request.getContextPath() %>/static/My97DatePicker/skin/datePicker.gif) no-repeat right!important;"> 
									</td>
									
									<td class="td-left">服务商状态：</td>
									<td class="td-right">								   
									...
									<td class="td-right" > 
										<span class="input-icon">
											<input type="text"  name="custManager" id="custManager" placeholder="请输入姓名" value="${queryBean.custManager }">
											<i class="icon-leaf blue"></i>
										</span>
									</td>
									<tr>
									<td colspan="8" align="center">
										<span class="input-group-btn">
											<gyzbadmin:function url="<%=RegistAuditPath.BASE + RegistAuditPath.LIST %>">
								...
								<table id="sample-table-2" class="list-table">
									...
									<c:forEach items="${merchantList }" var="merchant">
										<tr class="merchant" id="merchant">
											...
											<td>	
												<c:choose>   
							  						<c:when test="${merchant.auditState=='01' && merchant.merchantState =='04'}">  									  
