			...
			<div class="main-content">
				...
				<div class="page-content">
					<div class="row">
									...
									<td colspan="6" align="center">
										<span class="input-group-btn">
										   ...
										   <span class="input-group-btn" style="display:inline;">
												<a onclick="GoPayList();" href="javascript:volid(0);" class="btn btn-purple btn-sm">返回</a> 
											</span> 
						...
						<div class="list-table-header">代付失败明细列表</div>
							...
							<table id="sample-table-2" class="list-table" style="overflow:scroll;">
								<thead>
									<tr >
		                                        ...
		                                        <th width="5%">序号</th>
		                                		<th width="5%">收款人名称</th>
		                                		<th width="10%">收款人账号</th>
		                                		<th width="15%">收款人银行</th>
		                                		<th width="5%">收款金额</th>
			                                	<th width="5%">状态</th>
			                                	<th width="25%">处理摘要</th>
			                                	<th width="5%">创建人</th>
								...
								<tbody>
									...
									<c:forEach items="${paymentBatInfos}" var="payment" varStatus="i">
									  ...
									  <td>
										 ...
										 <c:if test="${payment.status =='04'}">
											<font color="red">付款失败 </font>		
										 </c:if>			
										 </td>
										  <td>${payment.procMemo}</td>
										  ...
										  <td>${payment.createTime}</td>
											<td>	
											  ...
											  <c:if test="${payment.status =='04'}">
											  <button type="button" onclick="dealPaymet(this)"  data-toggle='modal' data-target="#showHistory" disabled="disabled" class="btn  btn-xs" >处理代付查询</button> 
											 </c:if>
											</td>
										</tr>
									</c:forEach>
  ...
  /**处理代付请求**/
	 function dealPaymet(obj){
		 ...
		 $.ajax({
				type:"POST",
				dataType:"json",
				url:window.Constants.ContextPath +'<%=PaymentManagerPath.BASE + PaymentManagerPath.STATUS %>',
				data:
				 {
					"batNo" 	: batNo,
					"rowNo"     :rowNo
				  ...
				  success:function(data){
						if(data.result=="SUCCESS"){
							$.gyzbadmin.alertSuccess("查询完成！",function(){
							...
							$.gyzbadmin.alertFailure("查询失败！"+data.message);
...
//导出数据
  ...
  function GoPayList() {
	  window.location.href="/paymentmanager/toPayList" ;
