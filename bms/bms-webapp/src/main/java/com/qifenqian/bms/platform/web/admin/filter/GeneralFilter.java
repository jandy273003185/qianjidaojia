package com.qifenqian.bms.platform.web.admin.filter;
...
public class GeneralFilter implements Filter {
	...
	private void setCurrentUser(HttpServletRequest httpServletRequest) {
		String loginId = httpServletRequest.getParameter("loginId");
		if(StringUtils.isBlank(loginId)){
			return;
		}
		if (null == redisTemplateUtil) {
			BeanFactory factory = WebApplicationContextUtils.getRequiredWebApplicationContext(httpServletRequest.getServletContext());
			redisTemplateUtil = (RedisTemplateUtil) factory.getBean("redisTemplateUtil");
		}
		User user = WebUtils.getUserInfo();
		if (user == null) {
			user = redisTemplateUtil.getCacheObject(CacheConstants.CURRENT_USER_ID_TEMP + loginId, User.class);
			WebUtils.users.set(user);
		}
	}
	/**
	 * 过滤
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		setCurrentUser(httpServletRequest);
		// 定义常规静态资源请求不输出日志
		...
		try {
			if (skip) {
				...
				LOGGER.debug("[request ip:{}, path:{}] begin", httpServletRequest.getRemoteHost(), requestUri);
				...
				try {
					...
					if (response.isCommitted()) {
						...
						LOGGER.info("[request ip:{}, path:{}] end", httpServletRequest.getRemoteHost(), requestUri);
						// 移除
						MDC.clear();
						return;
					}
				...
				LOGGER.debug("[request ip:{}, path:{}] end", httpServletRequest.getRemoteHost(), requestUri);
				// 移除
				MDC.clear();
			}
		} finally {
