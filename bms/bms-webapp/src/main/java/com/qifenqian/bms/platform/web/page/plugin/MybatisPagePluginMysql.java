...
/**
 * MyBatis针对mysql定制的分页插件
 * 
 * @project gyzb-platform-web
 * @fileName MybatisPagePluginMysql.java
 * @author huiquan.ma
 * @date 2015年11月18日
 * @memo
 */
...
public class MybatisPagePluginMysql implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (invocation.getTarget() instanceof RoutingStatementHandler) {
			...
			if (ThreadUtils.get(Constants.Platform.$_BY_PAGE) == Boolean.TRUE) {
				...
				String countSql = "select count(1) from (" + sql + ") TABLE_PAGE_COUNT"; //记录统计  
	...
	private String generatePageSql(String sql, PageInfo page) {
		...
		if (page != null) {
			...
			pageSql.append(sql);
			pageSql.append(" ) TABLE_PAGE_LIST LIMIT ");
			pageSql.append(page.getFirstIdx()-1);
			pageSql.append(" , ");
