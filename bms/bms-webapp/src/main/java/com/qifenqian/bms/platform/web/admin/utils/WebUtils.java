...
public class WebUtils {
	...
	public static void registerUserInfo(User user) {
		logger.info("user Thread id:{}", "" + Thread.currentThread().getId());
		users.set(user);

		sessions.set(requests.get().getSession());

		sessions.get().setAttribute(Constants.Platform.USER_INFO_IN_SESSION, user);
	}
	...
	public static User getUserInfo() {
		...
		if (user == null) {
			if (null != sessions.get()) {
				user = (User) sessions.get().getAttribute(Constants.Platform.USER_INFO_IN_SESSION);
				users.set(user);
			}
		}
		return user;
	...
	public static void destroy() {
		...
		if (request != null) {
			request.setAttribute(Constants.Platform.USER_IN_REQUEST_SCOPE, users.get());
	...
	public static void init(HttpServletRequest request, HttpServletResponse response) {
		...
		LinkedList<RequestLink> requestHistory = (LinkedList<RequestLink>) httpSession
				.getAttribute(Constants.Platform.REQUEST_HISTORY);

		// 请求映射(增强按照名称来返回页面的业务逻辑)
		Map<String, RequestLink> requestMap = (Map<String, RequestLink>) httpSession
				.getAttribute(Constants.Platform.REQUEST_MAP);
	...
	public static void setReturnLink(String linkId) {
		...
		Map<String, RequestLink> requestMap = (Map<String, RequestLink>) getHttpSession()
				.getAttribute(Constants.Platform.REQUEST_MAP);
	 ...
	 */
	public static void showReturnLinkWithLinkId(String returnLinkTitle, String linkId, Map<String, String[]> params) {
		Map<String, RequestLink> requestMap = (Map<String, RequestLink>) getHttpSession()
				.getAttribute(Constants.Platform.REQUEST_MAP);
		RequestLink returnLink = requestMap.get(linkId);
		if (returnLink == null) {
			logger.error("当前会话中并未保存链接ID为{}的链接信息", linkId);
			return;
		}
		if (params != null) {
		...
		List<ReturnLink> returnLinks = (List<ReturnLink>) request.getAttribute(Constants.Platform.RETURN_LINKS);
		if (returnLinks == null) {
	 ...
	 * @param params          返回链接参数
	 */
	public static void showReturnLink(String returnLinkTitle, String linkUri, Map<String, String[]> params) {
	...
	public static void disableShowReturnLastPage() {
		requests.get().setAttribute(Constants.Platform.DISABLE_LASTPAGE, Constants.Platform.DISABLE_LASTPAGE);
	}
	...
	public static void setSubmitToken() {
		...
		HttpServletRequest request = getHttpServletRequest();
		if (request == null) {
			logger.error("设置防重token时检测到当前线程未绑定HttpServletRequest对象");
		...
		if (session == null) {
			logger.error("设置防重token时检测到当前线程未绑定HttpSession对象");
			throw new IllegalArgumentException("当前线程未绑定HttpSession对象");
		}

		// 产生防重ID
		String token = RandomStringUtils.randomAlphanumeric(10);
		logger.info("针对请求{}响应页面前,产生防重token:{}", request.getRequestURI(), token);
	...
	public static void checkSubmitToken() {
		...
		if (sessionToken == null) {
			/*
			 * 本次请求视为非法请求,将会擦除最新的请求历史记录,否则返回上一页会再一次进行相同提交
			 * 这里没有考虑到ajax方式的防重.ajax方式的防重是不会记录请求的,所以无须删除
			 */
			LinkedList<RequestLink> links = (LinkedList<RequestLink>) sessions.get()
					.getAttribute(Constants.Platform.REQUEST_HISTORY);
			if (!links.isEmpty()) {
				links.remove(0);
			}
