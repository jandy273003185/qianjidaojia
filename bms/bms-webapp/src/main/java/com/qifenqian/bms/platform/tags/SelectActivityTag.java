 ...
 *
 */
public class SelectActivityTag extends SimpleTagSupport{
	...
	public void doTag() throws JspException, IOException {
		...
		List<ActivityBean> activityList = activityService.selectActivityBeanList(null);

		if(null != activityList && !activityList.isEmpty()) {
			for (ActivityBean activityBean : activityList) {
				if (activityBean.getActivityId().equals(defaultValue)) {
					jspWriter.write("<option selected='selected' value='" + activityBean.getActivityId() + "'>" + activityBean.getActivityName()+ "</option>");
				} else {
					jspWriter.write("<option value='" + activityBean.getActivityId()+ "'>"+activityBean.getActivityName() + "</option>");
				}
			}
		}
