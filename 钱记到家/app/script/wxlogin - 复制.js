...
if (iswx() && typeof sc != "undefined") {
    ...
    if (sc.auth.isLogin()) {// 登录情况下,需要去绑定
        ...
        if (sc.utils.isNullOrEmpty(userinfo)) { //如果未获取到微信用户信息
            ...
            if (sc.utils.isNullOrEmpty(userinfo.Openid)) {
                IsHashOpenId();
            } else {//微信手机cookie都存在，可能已经过期
                var tokenExpiresUtc = sc.auth.getTokenExpiresUtc();              
                if (sc.utils.isNullOrEmpty(tokenExpiresUtc)) { //不存在
                    BindLogin();
                } else {//已经过期
                    var d = new Date(tokenExpiresUtc).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');                    
                    var curDate = new Date(new Date()).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');                           
                    if (d <= curDate) {
                        BindLogin();
                    }
                    isWxLoginOver = true;
        ...
        var callback = function (data) {
            if (!sc.utils.isNullOrEmpty(data.data)) {
                if (!sc.utils.isNullOrEmpty(data.data.Token) && !sc.utils.isNullOrEmpty(data.data.UserId)) {
                    ...
                    sc.auth.setTokenExpiresUtc(data.data.TokenExpiresUtc);
                    //return false;
                } 
            }
            isWxLoginOver = true;
