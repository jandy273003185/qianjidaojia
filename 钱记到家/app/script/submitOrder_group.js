...
$(function() {
	...
	if(sc.utils.getQueryString("groupId")) {
		groupId = sc.utils.getQueryString("groupId");
	}
	if(sc.utils.getQueryString("groupRecordId")) {
		groupRecordId = sc.utils.getQueryString("groupRecordId");
	}

	hasDefaultAddress(); //获取默认地址
	getInvoiceList(); //获取发票抬头
...
function confirmationGroup() {
	var url = 'api/GroupBuy/ConfirmationGroup';
	var callback = function(data) {
		console.log(data);
		if(data.code === 2) {
			layer.open({
		...
		if(data.code == 0) {
			...
			$("#detailsSureOrder .m-pro .name").text(data.data.GroupProductName);
			$("#detailsSureOrder .skuType .type").text(skuTxt);
			$("#detailsSureOrder .m-pro .price .num").text(data.data.FightingPrice);
...
function createGroupOrder() {
	var url = 'api/GroupBuy/CreateGroupOrder';
	var callback = function(data) {
		console.log(data);
		if(data.code === 200) {
			layer.open({
			...
			setTimeout(function() {
				...
				window.location.href = "/pintuan_info.html?groupId=" + groupId + "&id=" + data.data.GroupRecordId + "";
			}, 1000);
			...
			if(data.code === 2) {
				...
				var urldata = encodeURIComponent("gotoType=2&groupId="+groupId+"&groupRecordId="+data.data.GroupRecordId+"&orderno=" + data.data.OrderNo);
