...
$(function() {
	...
	//关闭选择规格弹窗
	//	$('.shadebg,.shade .btn-sure,.shade .btn-close').click(function() {
	//		var that = $(this).parents('.shade');
	//		that.fadeOut();
	//		that.find('.shade-item').animate({
	//			bottom: "-100%"
	...
	//		str += '<li>';
	//		str += '<div class="box">';
	//		str += '<div class="img"><img src="images/code.jpg"/></div>';
	//		str += '<div class="right">';
	//		str += '<div class="txt">购买成功</div>';
	//		str += '<a href="javascript:;" class="btn sendcode_btn">发送二维码</a>';
	//		//				str +='<a href="javascript:;" class="btn sendcode_btn btn_disable">发送成功</a>';
	//		str += '</div>';
	//		str += '</div>';
	//		str += '</li>';
	//		str += '</ul>';
...
var shopId;
var isLimit = sc.utils.getQueryString("Limit");//是否是限量秒杀
var isOnebuy = sc.utils.getQueryString("Onebuy");//是否是一元购
//计算折扣
...
//				以下价格秒杀和一元购不显示
				if( !isLimit && !isOnebuy ){
					...
					strhtml1 +='<p class="Tags"><span class="rebate">'+toPercent(data.data.ProductPrice,data.data.MarketPrice)+'折</span>';
					strhtml1 +='<span class="counterPrice">￥'+data.data.MarketPrice+'</span>';
					}
					strhtml1 +='</p>';
					strhtml1 +='<span class="sale_ed">已售'+data.data.SalesVolume+'份</span>';
				}
			...
			else if(isOnebuy){//一元购
				$('#new_price').html('1.00'); //一元价		
				$('#old_price').html(data.data.ProductPrice); //原价
			}
			else{
